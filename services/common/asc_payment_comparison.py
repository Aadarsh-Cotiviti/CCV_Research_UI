# python -m services.common.asc_payment_comparison
"""
ASC Payment Comparison Data Loader
...
def parse_sheet_name(sheet_name: str) -> Tuple[str, str, str]:
    ...
    Args:
        sheet_name: Sheet name like "January 2024 ASC AA"
        
    Returns:
        Tuple of (month, year, asc_addendum)
    """
    # Pattern: Month YYYY ASC XX (e.g., "January 2024 ASC AA")
    pattern = r'^(\w+)\s+(\d{4})\s+ASC\s+([A-Z]{2})$'
    match = re.match(pattern, sheet_name, re.IGNORECASE)
    
    if match:
        month, year, asc = match.groups()
        return month, year, asc
...
def filter_years(sheet_names: List[str], start_year: int = 2024, end_year: int = 2026, january_only: bool = True) -> List[str]:
    ...
    for sheet_name in sheet_names:
        month, year, asc = parse_sheet_name(sheet_name)
...
def detect_header_row(excel_path: str, sheet_name: str, max_rows: int = 20) -> int:
    ...
    for idx, row in df_peek.iterrows():
        ...
        matches = sum([
            ...
            any('payment' in val and 'indicator' in val for val in row_values),
...
def load_payment_data(
    ...
    Returns:
        ...
        ['HCPCS Code', 'Payment Indicator', 'Payment Rate']
    """
    # Default path
...
def asc_get_payment_history_for_multiple_cpts(
    cpt_codes: List[str],
    excel_path: str = None,
    ...
    for sheet_name, df in payment_data.items():
        ...
        # Filter for specific Payment Indicator
        keep_indicators = ['A2','G2','J8','P2','P3','R2']
        cpt_rows = cpt_rows[cpt_rows['Payment Indicator'].isin(keep_indicators)]
        ...
        for _, row in cpt_rows.iterrows():
            record = {
                ...
                'ASC': asc,
                'Period': sheet_name,
                'Payment Indicator': row['Payment Indicator'],
...
def get_asc_payment_history_with_exclusions(
    cpt_codes: List[str],
    exclusion_category: str = 'ASC',
    excel_path: str = None,
    january_only: bool = True
) -> Dict:
    """
    ...
    Args:
        cpt_codes: List of CPT codes to look up
        exclusion_category: CMS exclusion category - 'APC', 'ASC', or 'PNPP' (default: 'ASC')
        excel_path: Path to Excel file. If None, uses default path.
        january_only: If True, only return January data for each year (default: True)
    ...
    payment_history_df = asc_get_payment_history_for_multiple_cpts(
        cpt_codes=cpt_codes,
...
if __name__ == "__main__":
    ...
    history = get_asc_payment_history_with_exclusions(
