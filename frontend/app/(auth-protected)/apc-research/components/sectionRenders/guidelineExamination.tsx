...
type GuidelineResult = components["schemas"]["GuidelineResult"];
type CPTDescription = components["schemas"]["CPTDescription"];

type GuidelinePayload =
  | { section_num: number; status: "success"; data: GuidelineResult | string }
  | { section_num: number; status: "error"; error: string };

const isGuidelineResult = (raw: unknown): raw is GuidelineResult => {
  if (!raw || typeof raw !== "object") return false;
  const candidate = raw as GuidelineResult;
  return (
...
const parseGuidelineContent = (content: string): GuidelineResult | null => {
  const tryParse = (raw: unknown, allowNested = true): GuidelineResult | null => {
    if (!raw || typeof raw !== "object") return null;

    if (isGuidelineResult(raw)) return raw;

    const maybePayload = raw as Partial<GuidelinePayload>;
    if ("status" in maybePayload) {
      ...
      if (typeof maybePayload.data === "string" && allowNested) {
        try {
          ...
          console.warn("Failed to parse nested guideline data", error);
          return null;
        }
      }

      if (maybePayload.data && typeof maybePayload.data === "object") {
        return isGuidelineResult(maybePayload.data) ? maybePayload.data : null;
      }
    }
  ...
  try {
    ...
    console.warn("Failed to parse guideline examination response", error);
    return null;
  }
...
const GuidelineContent: FC<{ data: GuidelineResult }> = ({ data }) => {
  ...
  return (
    <div className="select-text cursor-text">
      ...
      {/* CPT Codes Section */}
      <SectionHeading id="cpt-code-descriptions">
        <span className="flex items-center gap-2">
      ...
      <p className="font-semibold text-foreground mb-3">
        {sortedCptCodes.length} {sortedCptCodes.length === 1 ? "code" : "codes"} identified:
      </p>
...
const GuidelineExaminationRenderer: AssistantRenderer = ({ message }) => {
  const parsedData = useMemo(() => parseGuidelineContent(message.content), [message.content]);

  if (!parsedData) return <>{message.content}</>;

  return <GuidelineContent data={parsedData} />;
};

export default GuidelineExaminationRenderer;
