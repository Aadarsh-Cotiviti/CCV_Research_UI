"""
Guideline Examination View (Section 2)

This module renders the UI for Section 2 - Guideline Examination
"""

import streamlit as st
from services.guideline_examination_service import analyze_guideline_examination, load_cached_results
from .utils import render_accuracy_feedback, render_chat_interface, render_source_legend, format_text_with_source


def format_guideline_data(data):
    """
    Format guideline examination data into markdown for display
    ...
    if cpt_descriptions:
        ...
        for cpt_code in sorted(cpt_descriptions.keys()):
            ...
            # Format with source coloring using the same method as code_description_view
...
def render_section(cpt_code, model, session_id, idx=0):
    """
    Render Section 2 view with guideline examination analysis
    ...
    """
    # Section title
    st.markdown("## SECTION 2 - Guideline Examination")
    st.markdown("---")
    
    # Initialize session state for this section's results
    section_key = f"section_2_results_{cpt_code}"
    ...
    with col1:
        load_cache_btn = st.button("📦 Load Cache", key=f"load_cache_btn_s2_{idx}", use_container_width=True)
    
    with col2:
        regenerate_btn = st.button("🔄 Re-generate Analysis", key=f"regenerate_btn_s2_{idx}", use_container_width=True)
    ...
    if regenerate_btn:
        with st.spinner("Running fresh analysis..."):
            fresh_data = analyze_guideline_examination(cpt_code, model=model, use_cache=False)
    ...
    if display_data and display_data.get("analysis_content"):
        ...
        # Important notice about CPT guidelines licensing
        st.info("**Note:** CPT guidelines are proprietary, copyrighted materials currently pending licensing approval. All content in this section is LLM-generated and should be verified against official CPT documentation.")
        
        # Format and display all content together
        formatted_content = format_guideline_data(display_data)
