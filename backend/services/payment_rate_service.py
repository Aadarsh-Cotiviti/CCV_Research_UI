...
from services.common.payment_comparison import get_payment_history_for_cpt, get_payment_history_for_multiple_cpts
...
def build_payment_rate_prompt(target_cpt, payment_table_md, neighboring_codes=None):
    """
    Build prompt for Payment Rate Comparison analysis
    
    Args:
        target_cpt: Target CPT code to analyze
        payment_table_md: Payment history table in markdown format
...
def analyze_payment_rate_comparison(target_cpt, model="gpt-4.1-mini", use_cache=True):
    ...
    try:
        ...
        # Get payment history from local knowledge base
        ...
        payment_history_df = get_payment_history_for_multiple_cpts(all_codes, january_only=True)
        ...
        # Convert to dict with source metadata
        ...
        # Convert to markdown for LLM prompt
        payment_table_md = payment_history_df.to_markdown(index=False)
        print(f"✅ Found {len(payment_history_df)} payment records (January only) for {len(all_codes)} CPT code(s)")
        
        # Build prompt and query LLM
        print("\nGenerating payment rate analysis...")
        prompt = build_payment_rate_prompt(target_cpt, payment_table_md, neighboring_codes)
...
if __name__ == "__main__":
    ...
    result = analyze_payment_rate_comparison(test_cpt, model="gpt-4.1-mini", use_cache=False)
